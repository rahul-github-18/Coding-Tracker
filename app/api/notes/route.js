...
export async function GET(req) {
  try {
    ...
    const res = await query('SELECT * FROM notes WHERE topic_id = $1 ORDER BY id ASC', [topicId]);

    // Fetch user completion status for these notes
    const tasksRes = await query(
      "SELECT item_id, status, saved_for_later FROM user_tasks WHERE user_id = $1 AND item_type = 'note'",
      [user.id]
    );
    ...
    const enrichedNotes = res.rows.map(n => ({
      ...n,
